...
namespace Cryptography.HomeWork4
{
    ...
    public sealed class Rijndael
    {
        ...
        private static readonly int ColumnHeight = 4;
        ...
        public Rijndael(RijndaelBlockSizes blockSize, RijndaelKeySizes keySize, byte irreduciblePolynomial
            , byte[] key)
        {
            BlockSize = blockSize;
            KeySize = keySize;
        ...
        private static byte[] MixColumnsPolynomial => _mixColumnsPolynomial.Value;

        private static byte[] InvertedMixColumnsPolynomial => _invertedMixColumnsPolynomial.Value;

        private static byte[] RCon => _rCon.Value;
        ...
        {
            get => _GF256 ??
                throw new ArgumentNullException(nameof(GF256));
        ...
        private int ColumnsCount => (int)BlockSize / ColumnHeight;

        private byte[] SBox => _sBox.Value;

        private byte[] InvertedSBox => _invertedSBox.Value;
        ...
        private byte[][] RoundKeys => _roundKeys.Value;
        ...
        private byte[] CreateSBox()
        {
            ...
            foreach (byte i in Enumerable.Range(0, sBox.Length))
            {
        ...
        private byte[] CreateInvertedSBox()
        {
            ...
            foreach (byte i in Enumerable.Range(0, invertedSBox.Length))
            {
